...
| "How did we get to this state? Append-only log, activity feed, audit trail, game moves." | **Durable Streams client** | `@durable-streams/client` |
| "AI chat with persistent, resumable, multi-client-synced conversations." | **Durable Transport for AI SDKs** | `@durable-streams/tanstack-ai-transport` (TanStack AI) or `@durable-streams/aisdk-transport` (Vercel AI SDK) |
