...
- **T2a (NEW):** `POST /api/chat` returns `401 Unauthorized` — API key never transmitted to server. Root cause: `durableStreamConnection` has its own `headers` option (checked via dist source: `mergeHeaders(options.headers)` merged into the fetch). The `headers` prop passed to `useChat({ headers: ... })` is a TanStack AI concept that the durable transport ignores entirely. Network inspection confirms `x-api-key` is absent from the actual POST request. **Fix:** move `"x-api-key": settings.apiKey` into `durableStreamConnection({ headers: { "x-api-key": settings.apiKey } })` and add `settings.apiKey` to the `useMemo` dependency array.
